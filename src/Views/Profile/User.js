  ...
  const actualizarValor = async (input, valor) => {
    switch (input) {
      case "displayName":
        ...
        console.log(usuario);
        Alert.alert("Actualizar", "Nombre de Usuario modificado.");
        break;
      case "email":
        if (validarEmail(valor) && valor !== usuario.email) 
        {
          setEmail(valor);
          await actualizarEmailFirebase(email);
          await addRegistroEspecifico("Usuarios", usuario.uid, { email: email });
          Alert.alert("Actualizar", "Email de Usuario modificado.");
        }
        break;
    }
  };
